...
| `npm run test:e2e` | e2e — `test/crm-flow.e2e-spec.ts`: 회원가입/로그인/토큰 → 고객 CRUD → 상담 기록 CRUD(autoSummarize) → 일정 생성·완료·삭제 → AI 분석·다음행동·문자·대시보드·주간리포트 → 설정 조회/변경 → OAuth 미구성 401 → 디바이스 등록/다이제스트 → 테넌트 격리(404) → 회원탈퇴 |
